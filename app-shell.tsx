...
async function downloadTemplate(
   ...
   * =======================================================
   *
   * Do not call /api/download.
   *
   * The visitor is not authenticated inside Smart Point,
   * therefore we send them directly to Whop Checkout.
   *
   * The selected template and requested format are passed
   * through the checkout URL.
   */

  if (!isAuthenticated) {
    const checkoutUrl = new URL(
      "https://whop.com/checkout/plan_5mxur1ghQi3Vk",
    );

    checkoutUrl.searchParams.set(
      "smart_point_template_id",
      String(templateId),
    );

    checkoutUrl.searchParams.set(
      "smart_point_file_type",
      format,
    );

    checkoutUrl.searchParams.set(
      "smart_point_return_url",
   ...
   * =======================================================
   *
   * Existing download flow remains unchanged.
   */
  ...
  try {
    ...
    /*
     * This should normally not happen anymore for an
     * external visitor because the visitor is redirected
     * before reaching /api/download.
     *
     * It remains here as a safety fallback.
     */
    if (
      response.status === 401
    ) {
